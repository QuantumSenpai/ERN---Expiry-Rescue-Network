...
function ringGradient(beams: 1 | 2, colors: [string, string?] | undefined): string {
  const tail0 = colors?.[0] ?? "#2F4156";
  const head0 = colors?.[0] ?? "#2F4156";
  const stops = [
    "transparent 0deg",
    comet(tail0, head0, "#2F4156", 55, 0),
  ];
  if (beams === 2) {
    const c1 = colors?.[1] ?? "#2F4156";
    stops.push("transparent 198deg", comet(c1, c1, "#2F4156", 50, 198));
  }
...
function BorderBeamPanelBase({
  ...
  const staticMode = hydrated && systemReduced;
...
.${cls}:hover .mk-beam-ring {
  opacity: 1;
}
@media (forced-colors: active) {
  .${cls} .mk-beam-ring { display: none; }
  ...
  return (
    <div
      ...
      onFocus={surge}
      onBlur={settle}
      className={cn("relative w-full border border-border bg-background text-foreground p-6 rounded-[24px] sm:rounded-[32px] shadow-none", cls, className)}
      style={{
...
export function BorderBeamPanel(props: BorderBeamPanelProps) {
  return <BorderBeamPanelBase {...props} />;
}
